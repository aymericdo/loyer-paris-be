...
=> db.plainecommuneaddresses.deleteMany({ nom_commune: {$nin: [ 'Pierrefitte-sur-Seine', 'Aubervilliers', 'Saint-Denis', 'Villetaneuse', 'La Courneuve', 'Épinay-sur-Seine', 'Stains', 'L\'Île-Saint-Denis', 'Saint-Ouen-sur-Seine'] } })
=> db.plainecommuneaddresses.updateMany({}, [{ $set: { geometry: { type: 'Point', coordinates: [{ $toDouble: "$lon" }, { $toDouble: "$lat" }] } } }])
*/

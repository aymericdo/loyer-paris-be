...
=> db.estensembleaddresses.deleteMany({ nom_commune: {$nin: [ 'Bagnolet', 'Bobigny', 'Le Pré-Saint-Gervais', 'Les Lilas', 'Montreuil', 'Noisy-le-Sec', 'Pantin', 'Romainville' ] } })
=> db.estensembleaddresses.updateMany({}, [{ $set: { geometry: { type: 'Point', coordinates: [{ $toDouble: "$lon" }, { $toDouble: "$lat" }] } } }])
*/

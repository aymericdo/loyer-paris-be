...
import {
  getRelevantAdsData,
  getRelevantAdsDataTotalCount,
} from '@services/db/queries/get-relevants-ad'
...
export async function getRelevantAds(req: Request, res: Response) {
  ...
  const mainCity: AvailableMainCities | 'all' =
    (req.query.cityValue as AvailableMainCities | 'all') || null
  const city: AvailableCities | 'all' =
    (req.query.cityValue2 as AvailableCities | 'all') || null
  ...
  const priceRange: [number, number] = priceValue
    ?.split(',')
    ?.map((v) => +v)
    .splice(0, 2) as [number, number]
  const exceedingRange: [number, number] = exceedingValue
    ?.split(',')
    ?.map((v) => +v)
    .splice(0, 2) as [number, number]
  ...
  const isHouse: boolean =
    isHouseValue && isHouseValue != 'null' ? +isHouseValue === 1 : null

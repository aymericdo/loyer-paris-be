...
export function getWebsiteFilter(website?: string): {
  website: string | { $nin: typeof FUNNIEST_WEBSITES }
} {
...
): { district: { $in: AvailableCityZones } } | Record<string, never> {
...
export function getFurnitureFilter(
  hasFurniture: boolean,
): { hasFurniture: boolean } | Record<string, never> {
  if (hasFurniture === null) return {}

  return { hasFurniture }
}

export function getHouseFilter(
  isHouse: boolean,
): { isHouse: boolean } | Record<string, never> {
  if (isHouse === null) return {}

  return { isHouse }
}

export function getSurfaceFilter(
  surfaceRange: [number, number],
): { surface: { $gte: number; $lte: number } } | Record<string, never> {
...
export function getClassicSurfaceFilter():
...
): { price: { $gte: number; $lte: number } } | Record<string, never> {
...
export function getClassicPriceFilter():
  | { price: { $gte: number; $lte: number } }
  | Record<string, never> {
  return getPriceFilter([0, 10000])
}

export function getRoomFilter(
  roomRange: [number, number],
): { roomCount: { $gte: number; $lte: number } } | Record<string, never> {
...
export function getIsParticulierFilter(
  isParticulier: boolean,
):
  | { renter: 'Particulier' | { $ne: 'Particulier'; $exists: true } }
  | Record<string, never> {
  ...
  | {
      $expr: {
        $and: [
          {
            $lte: [
              { $subtract: ['$priceExcludingCharges', '$maxPrice'] },
              number,
            ]
          },
          {

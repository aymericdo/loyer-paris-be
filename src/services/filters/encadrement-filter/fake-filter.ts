...
export class FakeFilter extends FilterParent {
  ...
  async find(): Promise<FilteredResult> {
    ...
    const bestCandidates = rentList.filter(item => getScore(item) === maxScore)
  ...
  protected async isRoomCountMatch(rent: ObservatoireEncadrementItem): Promise<boolean> {
  ...
  protected async isYearBuiltMatch(rangeRent: ObservatoireEncadrementItem): Promise<boolean> {
  ...
  async isHasFurnitureMatch(_rangeRent: ObservatoireEncadrementItem): Promise<boolean> {
    return true
  }

  protected async isHasHouseMatch(rangeRent: ObservatoireEncadrementItem): Promise<boolean> {
  ...
  protected async mappingResult(rentList: ObservatoireEncadrementItem[]): Promise<FilteredResult[]> {
    return rentList
      .map((rent: ObservatoireEncadrementItem) => {
        const res = {
          ...
          isFurnished: null,
          roomCount: this.normalizedRoomCount(rent.nombre_pieces_homogene).toString(),
  ...
  private normalizedRoomCount(nombre_pieces_homogene: string): string | number {
    ...
    return match ?
      parseInt(match[1]) === 4 ? '4 et plus' : parseInt(match[1]) :
      ''
  }
}

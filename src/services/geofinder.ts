import { AddressDigger } from '@addressdiggers/addressdigger'
...
export interface GeoFinderOutput {
    address: string
    postalCode: string
    stations: string[]
...
export class GeoFinderService {
    ...
    async digInAd(): Promise<CleanAd> {
        const addressDigger = new AddressDigger(this.city)
        const { address, postalCode, stations, coordinates, blurryCoordinates } = addressDigger.digForAddress(this.ad)
        ...
        const yearBuilt = await this.digForYearBuilt(this.city === 'paris' ? coordinates : null)

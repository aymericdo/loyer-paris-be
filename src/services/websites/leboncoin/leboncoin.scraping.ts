...
export class LeboncoinScraping {
  static scrap(data: string): LeboncoinMapping {
    ...
    const hasCharges = document.querySelector(
      '[data-qa-id=adview_price] > div p.text-caption',
    )
    const cityLabel = [
      ...document.querySelectorAll(
        '#mainContent > div > div > nav > div > div > nav > ol > li',
      ),
    ]
      .map((el, index) => {
        return index !==
    ...
    const rooms = document.querySelector(
      '#grid > article div div[data-qa-id=criteria_item_rooms] > div > div div p',
    )
    ...
    const dpe = document.querySelector(
      '#grid > article div div[data-qa-id=criteria_item_energy_rate] .styles_active__2SvNh',
    )

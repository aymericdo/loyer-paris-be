...
function normalizeConstructionDate(epoqueConstruction: string): string {
  const text = epoqueConstruction.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '')
...
async function editJsonData(data: Record<string, string>[]): Promise<Record<string, string>[]> {
  return data.filter((element) => {
    return element.Zone_calcul !== '' && element.Type_habitat !== '' && element.loyer_median !== ''
  }).map((element) => {
    ...
    return {
      ...
      epoque_construction_homogene: normalizeConstructionDate(element.epoque_construction_homogene),
      nombre_pieces_homogene: normalizeNombrePieces(element.nombre_pieces_homogene),
      loyer_median: element.loyer_median,
      loyer_median_max: (parseInt(element.loyer_median) * 1.2).toFixed(2).toString(), // +20%
      loyer_median_min: (parseInt(element.loyer_median) / 1.2).toFixed(2).toString(), // -20%
...
// === Convert CSV to JSON ===
async function csvToJsonData(csvString: string): Promise<Record<string, string>[]> {
...
async function main(fileName: string, city = null) {
  ...
  const outputPath = path.resolve(__dirname, `./data/json-data/encadrements_${currentCity}_${year}.json`)
  ...
  console.log(`✅ Fichier json généré à : ${outputPath} (à glisser dans json-data)`)
  process.exit(0)
}

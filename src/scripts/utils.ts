...
export async function fetchObservatoiresDesLoyers(departement: string, observatoireNumber: string) {
...
export async function observatoireVerification(mainCity: string, observatoire: string, year: string): Promise<boolean> {
  ...
  return new Promise((resolve, reject) => {
    ...
    stream
      ...
      .on('headers', (headers) => {
        ...
        if (!cityColumnName) {
          resolved = true
          return reject(new Error(`Aucune colonne ville trouvée dans : ${headers.join(', ')}`))
        }
      })
      .on('data', (row) => {
...
export async function readCityList(observatoire: string, year: string): Promise<string[]> {
  const filePath = path.resolve(__dirname, './data', `L${observatoire}Zonage${year}.csv`)
  ...
  return new Promise((resolve, reject) => {
    ...
    fs.createReadStream(filePath, { encoding: 'utf8' })
      ...
      .on('headers', (headers) => {
        ...
        if (!cityColumnName) {
          return reject(new Error(`Aucune colonne "LIBCOM" détectée parmi : ${headers.join(', ')}`))
        }
      })
      .on('data', (row) => {
      ...
      .on('end', () => resolve(Array.from(cities).sort((a, b) => a.localeCompare(b))))
      .on('error', reject)
  })
}
    ...
    fs.createReadStream(filePath, { encoding: 'utf8' })
      ...
      .on('headers', (headers) => {
        ...
        if (!inseeColumnName || !cityColumnName) {
          return reject(
            new Error(`Colonnes requises manquantes. Trouvées : ${headers.join(', ')}`)
          )
        }
      })
      .on('data', (row) => {
...
export async function getZonesByInsee(observatoire: string, year: string): Promise<{ [city: string]: string[] }> {
  const filePath = path.resolve(__dirname, './data', `L${observatoire}Zonage${year}.csv`)
